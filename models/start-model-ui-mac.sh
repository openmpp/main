...
#
echo "bin/oms" -l localhost:0 -oms.ModelDir "$PUBLISH_DIR" -oms.ModelLogDir "$PUBLISH_DIR" -oms.UrlSaveTo "$OMS_URL_TICKLE" -oms.HomeDir "$PUBLISH_DIR" -oms.ModelDocDir "$PUBLISH_DIR"/doc -oms.AllowDownload -oms.AllowUpload -oms.AllowMicrodata -oms.PidSaveTo "$PUBLISH_DIR"/oms.pid.txt -oms.LogRequest -OpenM.LogFilePath "$PUBLISH_DIR"/oms.log | tee -a "$START_OMPP_UI_LOG"

"bin/oms" -l localhost:0 -oms.ModelDir "$PUBLISH_DIR" -oms.ModelLogDir "$PUBLISH_DIR" -oms.UrlSaveTo "$OMS_URL_TICKLE" -oms.HomeDir "$PUBLISH_DIR" -oms.ModelDocDir "$PUBLISH_DIR"/doc \
          -oms.AllowDownload -oms.AllowUpload -oms.AllowMicrodata -oms.PidSaveTo "$PUBLISH_DIR"/oms.pid.txt -oms.LogRequest -OpenM.LogFilePath "$PUBLISH_DIR"/oms.log \
