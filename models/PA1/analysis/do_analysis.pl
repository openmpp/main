...
# Control which steps are performed
my $do_initialise    = 1;
my $do_specify_runs  = 1;
my $do_simulate_runs = 1;
my $do_assemble_runs = 1;

# Control which values of Cases and Replicates are iterated
my @Cases_list = (10000, 100000, 1000000, 10000000, 100000000);
my @Replicates_list = (4, 8, 10, 16, 20, 32, 64, 100, 200, 400, 800, 1000);

if (0) {
	# Values for quick test
	@Cases_list = (10000000);
	@Replicates_list = (100);
}

my $log_RR_sigma      = 0.20; # For PA: The s.d. of RR with condition (from equation estimation)
my $Alpha_mu          = 0.032000;  # For PA: The mean of the sample of Alpha values (copied from [Alpha.xlsx]Distribution!B3)
...
if ($do_specify_runs) {
	{
		...
		for my $IntrvProp (0.00, $chosen_intrv_prop) {
			...
			for my $Replicates (@Replicates_list) {
