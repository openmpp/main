        ...
        case et_report_style::eReadable:
            {
                ...
                case et_msg_type::eEventOccurrence:
                {
                    ...
                    assert(global_time == current_time);
                    int padding = std::max<int>(0, name_colwidth - int(1 + strlen(entity_name) + strlen(event_name)));
                    theTrace->logFormatted("%13.6f   EVENT ********* %08d %s.%s%*s age=%.6f",
                        global_time,
                        entity_id,
                        entity_name,
                        event_name,
                ...
                case et_msg_type::eSelfSchedulingEventOccurrence:
                {
                    ...
                    assert(global_time == current_time);
                    int padding = std::max<int>(0, name_colwidth - int(1 + strlen(entity_name) + strlen(event_name)));
                    theTrace->logFormatted("%13.6f   EVENT(SS) ***** %08d %s.%s%*s age=%.6f",
                        global_time,
                        entity_id,
                        entity_name,
                ...
                case et_msg_type::eQueuedEvent:
                {
                    ...
                    int padding = std::max<int>(0, name_colwidth - int(1 + strlen(entity_name) + strlen(other_name)));
                    theTrace->logFormatted("%13.6f     %13.6f %08d     %s.%s%*s old=%.6f,new=%.6f",
                        global_time,
                        new_time,
                        entity_id,
                        entity_name,
                        other_name, // is name of time function, eg timeMigrationEvent
                        padding, "",
                        old_time,
                        new_time
                    );
                ...
                case et_msg_type::eEnterSimulation:
                {
                    ...
                    auto& entity_age = dbl2;
                    int padding = std::max<int>(0, name_colwidth - int(1 + strlen(entity_name) + strlen(other_name)));
                    theTrace->logFormatted("%13.6f  ENTER SIMULATION %08d %s.%s%*s time=%.6f,age=%.6f",
                        global_time,
                        entity_id,
                        entity_name,
                        other_name, // is "enter_simulation"
                        padding, "",
                        entity_time,
                        entity_age
                    );
                ...
                case et_msg_type::eExitSimulation:
                {
                    ...
                    auto& entity_age = dbl2;
                    int padding = std::max<int>(0, name_colwidth - int(1 + strlen(entity_name) + strlen(other_name)));
                    theTrace->logFormatted("%13.6f   EXIT SIMULATION %08d %s.%s%*s time=%.6f,age=%.6f",
                        global_time,
                        entity_id,
                        entity_name,
                        other_name,  // is "exit_simulation"
                        padding, "",
                        entity_time,
                        entity_age
                    );
