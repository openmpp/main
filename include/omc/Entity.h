...
public:
        ...
        et_msg_type msg_type)
    {
        ...
        if (msg_type == eEnterSimulation && !event_trace_show_enter_simulation) {
            // Entity enter simulation messages are disabled.
            return;
        }
        if (msg_type == eExitSimulation && !event_trace_show_exit_simulation) {
        ...
        if ((msg_type == eEventOccurrence || msg_type == eEventScheduledTime) && event_trace_selected_events.size() > 0 && event_trace_selected_events.count(event_id) == 0) {
            // Event is not in the specified event list.
            return;
        }
        if (msg_type == eEventScheduledTime && !event_trace_show_scheduling) {
            // Future event scheduled time messages are disabled.
            return;
        }
