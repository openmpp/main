...
void AgentSymbol::create_ss_event()
{
    ...
    ct += "// Reset the flag used to trigger event time recalculation." ;
    ct += av_ss_flag->name + " = false;" ;
    ct += "" ;
    ct += "// Compute the minimum next time of all of the self-scheduling derived attributes." ;
    ct += "Time et = time_infinite;" ;
    // Subsequent injected code will min each self-scheduling time against the local variable 'et'
    // A final line of injected code will return 'et'.  This final line is generated by finish_ss_event().
    ...
    ci += "// Set the flag to trigger event time recalculation." ;
    ci += av_ss_flag->name + " = true;" ;
    // Subsequent injected code will, for each self-scheduling agentvar whose time has come,
    // update its value and its next scheduled time.
    
    // Create the event
    // Ex. "om_time_BirthdayEvent"
    string event_name = "om_" + ss_implement_fn->name + "_om_event";
    //auto *sym = new AgentEventSymbol(event_name, this, ss_time_fn, ss_implement_fn, 251, decl_loc);
