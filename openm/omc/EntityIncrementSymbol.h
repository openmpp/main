...
public:
     ...
     */
    EntityIncrementSymbol(const string member_name, const Symbol * ent, const EntityTableSymbol *table, omc::location decl_loc = omc::location())
        : EntityDataMemberSymbol(member_name, ent, UnknownTypeSymbol::find(), decl_loc)
        , table(table)
    {
    }
