...
void CodeGen::do_preamble()
{
    ...
    // om_initializers.cpp
    z += doxygen(
                "@file   om_initializers.cpp",
                "Definitions for internal parameters for model '" + model_symbol->name + "' - Generated by openM++ compiler",
                " ",
                " Model version = " + version_symbol->formatted_value(),
                " Time stamp = " + model_symbol->time_stamp
                );
...
void CodeGen::do_parameters()
{
	...
	z += "// model parameters (internal)";
    for ( auto parameter : Symbol::pp_all_parameters ) {
        if (parameter->initializer_list.empty()) {
            c += parameter->cxx_definition_global();
        }
        else {
            // place definition in the dedicated initializer C++ module
