...
int main(int argc, char * argv[])
{
    ...
    try {
        ...
        // from this list.  This provides a persistent pointer in the file member
        // of yy::position objects generated by the parser.
        list<string> source_files_long;
        ...
        for (const string & name : source_files) {
            try {
                ...
                string in_stem = name;
                string in_ext = "";
                if (endWithNoCase(name, ".mpp")) {
                    in_ext = ".mpp";
                    in_stem = name.substr(0, name.length() - 4);
                }
                if (endWithNoCase(name, ".ompp")) {
                    in_ext = ".ompp";
