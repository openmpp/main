...
template<> void ModelInsertSql::insertSql<ModelDicRow>(const ModelDicRow & i_row, ModelSqlWriter & io_wr)
{
    ...
    if (i_row.subPrefix.length() > OM_DB_TABLE_TYPE_PREFIX_LEN) throw DbException("invalid (too long) subsample tables prefix: %s", i_row.subPrefix.c_str());
    ...
    if (i_row.paramPrefix == i_row.setPrefix || i_row.paramPrefix == i_row.subPrefix || i_row.paramPrefix == i_row.valuePrefix ||
        i_row.setPrefix == i_row.subPrefix || i_row.setPrefix == i_row.valuePrefix || i_row.subPrefix == i_row.valuePrefix)
        throw DbException(
        "invalid (not unique) table prefixes: %s %s %s %s, model name: %s",
        i_row.paramPrefix.c_str(), i_row.setPrefix.c_str(), i_row.subPrefix.c_str(), i_row.valuePrefix.c_str(), i_row.name.c_str()
        );
    ...
    io_wr.writeQuoted(i_row.subPrefix, true);
