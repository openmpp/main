...
# Script to export the output tables of a Modgen output database to CSV format

use strict;
use warnings;

my $script_name = 'modgen_export_csv';
...
use common_windows qw(
				modgen_tables_to_csv
		    );

if (! -s $db) {
	logmsg error, $script_name, "Modgen database '${db}' not found\n";
	exit -1;
}
...
my $failure = modgen_tables_to_csv($db, $out_dir);
if ($failure) {
