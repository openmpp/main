...
for my $model_dir (@model_dirs) {
	...
	for (my $j = 0; $j <= $#flavours; $j++) {
		...
		if ($flavour eq 'modgen') {
			...
			my $grid_computing = get_user_macro($model_props, 'GRID_COMPUTING');
			...
			my $build_suffix = "";
			$build_suffix .= 'D' if $ompp_linux_config eq 'debug';
			...
			# Save copy of generated C++ source code
			my $linux_generated_code_dir = "${project_dir}/build/${ompp_linux_config}/src";
